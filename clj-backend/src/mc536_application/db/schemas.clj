...
(def entidade-empresa
  [:map
   ...
   [:saldo_auxiliar {:optional true} decimal?]])
...
(def vaga_emprego
  [:map
   ...
   [:remuneracao {:optional true} decimal?]])
...
(def entidade-pessoa-fisica
  [:map
   ...
   [:email {:optional true} string?]
   [:federacao [:string? {:min 2 :max 2}]]
   [:renda_familiar {:optional true} decimal?]
...
(def auxilio
  [:map
   ...
   [:valor_mensal {:optional true} decimal?]
   [:valor_total {:optional true} decimal?]
